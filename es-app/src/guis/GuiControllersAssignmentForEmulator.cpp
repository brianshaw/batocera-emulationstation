...
GuiControllersAssignmentForEmulator::GuiControllersAssignmentForEmulator(Window* wnd, std::string systemName) : GuiSettings(wnd, controllers_settings_label.c_str())
{
	...
	for (int player = 0; player < MAX_PLAYERS; player++)
	{
		...
		std::string configuratedName = Settings::getInstance()->getString(Utils::String::format("%s INPUT P%iNAME",systemName.c_str() , player + 1));
		std::string configuratedGuid = Settings::getInstance()->getString(Utils::String::format("%s INPUT P%iGUID",systemName.c_str() , player + 1));
		std::string configuratedPath = Settings::getInstance()->getString(Utils::String::format("%s INPUT P%iPATH",systemName.c_str() , player + 1));
